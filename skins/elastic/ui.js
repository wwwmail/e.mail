...
function rcube_elastic_ui()
{
    ...
    function bootstrap_style(context)
    {
        ...
        $('#message-objects', context).children(':not(.ui.alert)').each(function() {
            ...
            $('a', this).addClass('btn btn-primary');
             ...
             $('#remote-objects-message').attr('onclick', "rcmail.command('load-remote', true)");
          $('#remote-objects-message').css('cursor','pointer');
    ...
    function show_list(scroll)
    {
        if (!layout.list.length && !layout.sidebar.length) {
            ...
            layout.list.addClass('show-list-content');
    ...
    function recipient_input(obj)
    {
        var list, input, ac_props,
            ...
            insert_recipient = function(name, email, replace) {
               ...
               contactName = recipient_input_name(name || email),
                 FirstLast = '';
                if(contactName){
                     contactName = contactName[0].split(" ", 2);

                    if(contactName[0]){
                        FirstLast =  contactName[0][0];
                      
                    }

                }
               
             var letter_element = $('<span class="letter">').html(recipient_input_name(FirstLast)),
                  name_element = $('<span class="name">').html(recipient_input_name(name || email))
                    ...
                    link = $('<a>').attr({'class': 'button icon remove'})
                        .click(function() {
							 if($(".recipient-input li.recipient").length < 2){
								$('.btn.btn-primary.send').removeClass('can-send');
							}
                            recipient.remove();
                            apply_func();
                            input.focus();
                ...
                if (name) {
				  ...
				  $('.btn.btn-primary.send').addClass('can-send');
				  
                email_element.text((name ? email : '') + ',');
                recipient.attr('title', name ? (name + email) : null)
                    .append([letter_element, name_element, email_element, link])
           ...
           tabs = $('<ul class="nav nav-tabs">')
                ...
                .append($('<li class="nav-item">')
                    .append($('<a class="nav-link mode-plain" href="#">')
                        .text(rcmail.gettext('plaintoggle')))).hide();
