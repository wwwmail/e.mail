<?php
$api = 'https://mailapi.mail.cz';
$domenApi = 'https://mail.cz';
/**
 +-------------------------------------------------------------------------+
...
if(isset($_COOKIE['token']) &&  !isset($_SESSION['user_id'])   ){
        ...
        if(isset($_COOKIE['token'])){
            ...
            $ch = curl_init($api.'/auth/validate-token');
            ...
            $resultUserInfo = curl_exec($ch);
            curl_close($ch);
            $resultUserInfo = json_decode($resultUserInfo);



            if($resultUserInfo->success == true){

                require_once 'yii/index.php';

    $data = common\models\Connection::findOne(['email' => trim($resultUserInfo->data->profile->email)]);
  ...
  //  $request_valid = $_SESSION['temp'] && $RCMAIL->check_request();
   ...
   // $RCMAIL->kill_session(); // do not kill session
...
}elseif (isset($_COOKIE['token']) && !isset($_GET['_task'])) {
...
$RCMAIL->kill_session();
...
}else if(!isset($_COOKIE['token']) &&  isset($_SESSION['user_id'])   ){
    ...
    if(isset($_COOKIE['token'])){
     ...
     //setcookie ("token", $resultUserInfo->data->access_token, time()+3600);
  //  echo 4; die;
